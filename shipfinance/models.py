...
# ---------------------------------------------------------------------------
# Secure group filter (lockout for members with overdue invoices)
# ---------------------------------------------------------------------------

class NoOverdueShipFinanceFilter(models.Model):
    """Secure-group filter: fails members who have overdue shipfinance invoices.

    Attach this to the trusted ship-finance access group. Members behind on
    payments auto-lose access to new rentals/finance until they settle.
    Reuses the invoices plugin's Invoice model for overdue detection.
    """

    name = models.CharField(max_length=500, default="No Overdue Ship Finance")
    description = models.CharField(
        max_length=500, default="Fails members with overdue ship finance invoices")
    swap_logic = models.BooleanField(default=False)

    class Meta:
        verbose_name = "Smart Filter: No Overdue Ship Finance"
    ...
    def audit_filter(self, users):
        ...
        overdue = Invoice.objects.filter(
            character__in=co.values_list("character"),
